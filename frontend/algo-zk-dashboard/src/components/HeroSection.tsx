...
export default function HeroSection() {
  ...
  const fetchPrice = async () => {
    try {
      ...
      const realVolume =
        data.sources?.find((s: { volume_24h: number }) => s.volume_24h > 0)
          ?.volume_24h || 0;
      const realChange =
        data.sources?.find((s: { change_24h: number }) => s.change_24h)
          ?.change_24h || 0;
  ...
  return (
    <div className="text-center space-y-8">
      ...
      >
        <Badge
          variant="secondary"
          className="mb-4 bg-purple-500/20 text-purple-200 border-purple-500/30"
        >
        ...
        <p className="text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto">
          ...
          <span className="text-purple-400 font-semibold">
            {" "}
            Zero-Knowledge ML ensemble models
          </span>
        </p>
      </motion.div>
      ...
      >
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-gray-400">
            Model weights hidden with ZK-SNARKs
          </p>
        </div>
